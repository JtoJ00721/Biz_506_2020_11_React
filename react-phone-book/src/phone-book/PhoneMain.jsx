...
/*
  id, name, number의 요소를 담는 JSON 객체를 3개 포함하는 
  배열을 선언과 생성
  phoneBook배열을 생성
*/
const PhoneMain = () => {
  // react에서 변수를 선언할때
  // state상태로 변수를 선언한다
  // state형태는 react 엔진이 이 변수의 값이 변하는가를 감시하고 있다가
  // 어느순간 값이 변화가 되면 화면을 다시 랜더링을 수행한다.
  // useState() 함수는 변수를 선언하고 state 형태로 React 엔진에 등록하는
  //    일을 수행한다.
  // useState(값) : state형 변수를 선언하고 토기화시키는 일을 수행
  ...
  /*
    useRef() 함수를 사용하여 Ref 객체를 선언하면
    current하는 속성이 자동으로 생성되고 초기화 된다
  ...
  */
  ...
  변수와 함수를 전달하는 방법은 같은 방법으로 하면 된다.
  ...
  */
  return (
    <>
      <PhoneInsert onClickSave={onClickSave} phoneBook={phoneBook} />
