...
const PhoneInsert = ({ onClickSave }) => {
  ...
  /*
    ...
    React에서는 키보드에서 입력한 문자열을 이미 선언된 state 변수에 담아서
    필요한 곳으로 이동을 하는 방식을 사용한다.

    React에서는 input box에 onChange이벤트를 설정하여 그 이벤트가 발생할떄
    사용자가 키보드로 입력한 문자열을 state변수에 저장해주는 코드를 작성해야 한다.

    input box에 클릭을 하고 키보드로 문자를 입력하면 한글자마다 (e) 매개변수의
    e.target.value값에 일단 저장이 된다.
    e.target.value값에 저장된 문자열을 setname() 함수를 사용하여 name state변수에
    저장을 해준다.
  */
  const nameChange = (e) => {
  ...
  /*
    onClick(또는 여러가지 이벤트) 핸들러 함수를 parents 컴포ㅈ넌트로부터 전달받아 사용할때
    그냥 단순한 이벤트 핸들러라면 onClik = {onClick} 처럼 사용하면 된다.
    ...
    화면이  Rendering될때 수시로 onClickSave() 함수가 실행되어 React가 폭주하는 
    현상이 발생을 한다.
    함수를 만들고 그 함수 내에서 다시 이벤트 핸들러를 호출해주는 2중함수 방식으로
    코딩을 해야 한다.
  */
